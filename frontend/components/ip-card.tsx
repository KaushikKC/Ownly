...
export default function IPCard({ id, title, thumbnail, collaborators, licenseType, status, onSelect }: IPCardProps) {
  ...
  const handleClick = () => {
    if (onSelect) {
      onSelect({ id, title, thumbnail, collaborators, licenseType, status })
    }
  }
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <Badge className={`text-xs ${getLicenseColor(licenseType)}`}>{licenseType}</Badge>
